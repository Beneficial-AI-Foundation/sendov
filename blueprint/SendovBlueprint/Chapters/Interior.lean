...
Lemma 1; those give the raw polar inequality $`(1Q)` and the raw origin
...
# The four identities of Lemma 1
...
`Sendov.centroid_identity`. Uses {uses "in_factor"}[].
:::
...
from $`p'(0)` computed two ways. In Lean: `Sendov.second_origin_identity`. Uses
...
`Sendov.first_origin_identity`. Uses {uses "in_factor"}[].
:::
...
segment from $`a` to $`1/a`. In Lean: `Sendov.polar_identity`. Uses
{uses "in_factor"}[].
:::
...
In Lean: `Sendov.one_le_integral_prod_norm` (`Sendov/Analytic/Polar.lean`).
...
In Lean: `Sendov.one_le_integral_Ppolar`. Uses {uses "in_polar_star"}[],
...
In Lean: `Sendov.defect` (`Sendov/Analytic/Defect.lean`).
:::
...
In Lean: `Sendov.norm_deriv_add_le`, via `Sendov.sumEraseProdMap_norm_le`. Uses
...
In Lean: `Sendov.one_le_tri`. Uses {uses "in_error_bound"}[].
:::
...
Lean: `Sendov.Jsum_estimate` (`Sendov/Analytic/Jsum.lean`). Uses
...
only place the hypothesis $`n \ge 5` is used.* In Lean: `Sendov.grow`.
:::
...
In Lean: `Sendov.origin_exact` (`Sendov/Analytic/OriginExact.lean`). Uses
...
`Sendov.polar_exp` (`Sendov/Reduction/Polar.lean`). Uses {uses "in_1Q"}[].
:::
...
`Sendov.log_le_alpha_mul` (`Sendov/Reduction/BetaBound.lean`). Uses
...
In Lean: `Sendov.alpha_le_seventeen` (`Sendov/Reduction/Alpha17.lean`).
...
`Sendov.one_le_of_origin` (`Sendov/Reduction/Simplified.lean`). *(Deviation
...
(`Sendov/Reduction/Stat.lean`). Uses {uses "in_one_le"}[],
...
`Sendov.sendov_interior` (`Sendov/Interior.lean`). Uses {uses "in_factor"}[],
...
at most one. In Lean: `Sendov.sendov_center`. Uses {uses "in_factor"}[].
:::
